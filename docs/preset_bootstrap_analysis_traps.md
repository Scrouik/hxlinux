...
| Fichier | Rôle |
|---|---|
| `src-tauri/src/helix/phase4_state.rs` | FSM phase 4 : préambule (`Waiting68o`), trailer (`WaitingDump`), PHASE B |
| `src-tauri/src/helix/preset_dump_stream_ack.rs` | ACK chunks dump sur `editor_ed03_lane` ; `HX_DUMP_ACK_LANE` |
| `src-tauri/src/helix/amorcage.rs` | Enchaînement amorçage → requêtes noms / preset actif |
| `src-tauri/src/helix/mod.rs` | Orchestration session, gate phase 4 (3500 ms) |

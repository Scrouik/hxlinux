...
- **Amp+Cab IR** (onglets Amp / Cab) : bulk `c319` + `<amp> 1a <cab>` ; `dualPart` `amp`/`cab`, index param **local** ; focus cab = `1d` ; live write cab `pp=03` ; **replace cab picker** = `focus → ed:08 → bulk` — validé HW juin 2026. **Doc :** [Amp_cab_fonctionnement_no_legacy.md](docs/Amp_cab_fonctionnement_no_legacy.md).

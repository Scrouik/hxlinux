...
Todo : 
  * ⬜ **`HX_ModelUsbAssign.json`** : compléter les **autres modèles** (captures + entrées) ; **revoir la structure** — champs **`edOpcode`** / **`bulkKind`** (et **`chainHexHint`**) non utilisés par le chargeur Rust (**seuls** `id`, `variant`, `bulkHex`) ; picker TS utilise `name` / `category` / `subCategory`. **Importer `basedOn`** depuis **`HX_ModelCatalog.json`**. **Note** : fusion USB long `chainHex` lit encore le catalogue ; **`chainHexHint`** non branché — et ordre picker = ordre fichier → voir **`hardwareOrder`** / **`TODO.md`**.

...
- **Amp+Cab IR** (onglets Amp / Cab) : bulk `c319` + `<amp> 1a <cab>` ; `dualPart` `amp`/`cab`, index param **local** ; focus cab = `1d` ; live write cab `pp=03` ; **replace cab picker** = `focus → ed:08 → bulk` (catalogue, pas `preset_data`). **Doc :** [Amp_cab_fonctionnement_no_legacy.md](docs/Amp_cab_fonctionnement_no_legacy.md).
- **Amp+Cab Legacy** (`amp+cab-legacy`) : focus cab `1b`, params `pp=08` ; **replace cab picker** = **`ef → f0 → bulk`** (même séquence que assign initial ; octets bulk 14–15 = `02 00`) — validé HW juin 2026. **Doc :** [Amp_cab_fonctionnement_legacy.md](docs/Amp_cab_fonctionnement_legacy.md).

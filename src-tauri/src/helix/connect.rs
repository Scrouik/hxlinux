...
pub fn connect_sequence(
    ...
    println!("ENVOI ack réinit x1");

    // 15. Attendre confirmation x1
    wait_raw(events, |d| d.len() >= 20 && d[4] == 0xef && d[6] == 0x01 && (d[18] == 0x02 || d[18] == 0x05));
    println!("Confirmation x1 reçue !");

    // 16. Démarrage lecture preset courant
    helix.write(&[0x19,0x00,0x00,0x18,0x80,0x10,0xed,0x03,0x00,0x04,0x00,0x04,0x1a,0x1e,0x00,0x00,0x01,0x00,0x06,0x00,0x09,0x00,0x00,0x00,0x83,0x66,0xcd,0x04,0x04,0x64,0x17,0x65,0xc0,0x00,0x00,0x00])?;
    helix.write(&[0x08,0x00,0x00,0x18,0x01,0x10,0xef,0x03,0x00,0x03,0x00,0x08,0x72,0x1e,0x00,0x00])?;
    println!("ENVOI démarrage lecture preset");

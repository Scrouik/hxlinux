...
- [ ] **Chargement preset sans flags debug** : la lecture des presets au bootstrap **échouait** sans lane couplée (graine figée `0x1c7e` vs `0x6cbd`). **Corrigé (juin 2026)** : mode couplé **ON par défaut** (`HX_PULL_COUPLE_LANE=0` pour le témoin figé). Reste à valider bootstrap preset en session « normale » sans autres env de trace ; gate `editor_ready` sur le pull si besoin.
